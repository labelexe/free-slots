				...
				<section>
					<div class="taxonomy-archive-intro">
						<div class="taxonomy-archive-content">
							...
							<?php }else{?>
								<?php echo vsprintf(__('<h2>%s Gaming License</h2><p>%s Gaming License is considered as one of the most authoritative regulators in the gaming industry. Many online casinos are operating from within its jurisdiction. The presence of %s Gaming License is not an empty word. Once players see %s Gaming License mentioned on the casino’s website, they can take it as a guarantee of safety and security.</p>', 'websitelangid'), array($term_name, $term_name, $term_name, $term_name));?>
							<?php }?>
						</div>
			...
			<div class="align">
				<section>
					<header>
						...
						<h1><?php echo vsprintf(__('Casinos that Work Under %s License in %s', 'websitelangid'), array($queried_object->name, $country_native_name));?></h1>
						<p><?php echo __('Please Rate Our Casinos Collection', 'websitelangid');?></p>
						<?php echo do_shortcode('[gdrts_stars_rating type="terms.casinos_licenses" id="'.$term_id_original.'"]');?>
					</header>
					<?php
					if(is_paged()){
						if($_POST){
