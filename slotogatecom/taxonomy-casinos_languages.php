				...
				<section>
					<div class="taxonomy-archive-intro">
						<div class="taxonomy-archive-content">
							...
							<?php }else{?>
								<?php echo vsprintf(__('<h2>%s</h2><p>The gambling market keeps expanding its boundaries, which makes online casinos support more foreign languages like %s. Apart from the casino interface translated into %s, the customer service can also be catered in this language. This means that %s-speaking players can enjoy a full range of features without using Google Translator. No need to say how convenient it can be!</p>', 'websitelangid'), array($term_name, $term_name, $term_name, $term_name));?>
							<?php }?>
						</div>
			...
			<div class="align">
				<section>
					<header>
						...
						<h1><?php echo vsprintf(__('Casinos that Support %s Language in %s', 'websitelangid'), array($queried_object->name, $country_native_name));?></h1>
						<p><?php echo __('Please Rate Our Casinos Collection', 'websitelangid');?></p>
						<?php echo do_shortcode('[gdrts_stars_rating type="terms.casinos_languages" id="'.$term_id_original.'"]');?>
					</header>
					<?php
					if(is_paged()){
						if($_POST){
...
?>
		...
		<div class="content-zone linking-line float">
			<div class="align">
				<aside>
					<header>
						<h5><?php echo __('Other Casino Languages', 'websitelangid');?></h5>
					</header>
