...
<?php $country_detect = do_shortcode('[useriploc type="country"]');?>
<?php
			...
			<div class="align">
				<section>
					<header>
						...
						<h3><?php $country_term_name = get_field('country_native_name', $taxonomy.'_'.$term_id); if($country_term_name){$term_name = $country_term_name;}else{$term_name = $queried_object->name;} echo sprintf(__('Casino Bonuses Available in %s', 'websitelangid'), $term_name);?></h3>
						<p><?php echo __('Please Rate Our Bonuses Collection', 'websitelangid');?></p>
						<?php echo do_shortcode('[gdrts_stars_rating type="terms.bonuses_countries" id="'.$term_id_original.'"]');?>
					</header>
					<?php
					if(is_paged()){
						if($_POST){
					...
					<form class="filter-form" method="post">
						<div class="filter-infoblock">
							<p><span id="ItemsCount"></span> <?php echo __('Bonuses Found for Players from', 'websitelangid');?> <?php single_term_title();?></p>
						</div>
					...
					/*CURRENT TERM*/
					$country = array('taxonomy'=>$taxonomy, 'field'=>'id', 'terms'=>$term_id);
					
					$args = array('post_type'=>'bonuses', 'orderby'=>$bonuses_orderby, 'order'=>$bonuses_order, 'paged'=>$paged, 'tax_query'=>array($country));
					query_posts($args);
