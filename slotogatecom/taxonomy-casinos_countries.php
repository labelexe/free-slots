...
$country_term_name = get_field('country_native_name', $taxonomy.'_'.$term_id); if($country_term_name){$term_name = $country_term_name;}else{$term_name = $queried_object->name;}
?>
<?php
				...
				<section>
					<div class="taxonomy-archive-intro">
						<div class="taxonomy-archive-content">
							...
							<?php }else{?>
								<?php echo vsprintf(__('<h2>%s</h2><p>Online gambling is popular in %s. So, gambling operators are more than happy to accept local players. They offer a wide range of slot machines, table games, and live dealer games. Players from %s can play discreetly and enjoy a lot of benefits. If you are a gambler from %s, you have some opportunities to catch.</p>', 'websitelangid'), array($term_name, $term_name, $term_name, $term_name));?>
							<?php }?>
						</div>
			...
			<div class="align">
				<section>
					<header>
						...
						<h1><?php echo sprintf(__('%s Casino Reviews & Ratings', 'websitelangid'), $term_name);?></h1>
						<p><?php echo __('Please Rate Our Casinos Collection', 'websitelangid');?></p>
						<?php echo do_shortcode('[gdrts_stars_rating type="terms.casinos_countries" id="'.$term_id_original.'"]');?>
					</header>
					<?php
					if(is_paged()){
						if($_POST){
...
jQuery('#ToggleFiltersButton').click(function(){
	...
	jQuery('#ToggleFiltersContent').toggleClass('active');
						...
						<div class="filter-infoblock">
							<p><span id="ItemsCount"></span> <?php echo __('Casinos Found for Players from', 'websitelangid');?> <?php single_term_title();?></p>
						</div>
...
jQuery(function(){
	jQuery(".clear-btn").click(function(){
		...
		jQuery('.casinos-filter').submit();
					...
					$args = array('post_type'=>'casinos', 'meta_key'=>$casinos_meta_key, 'orderby'=>$casinos_orderby, 'order'=>$casinos_order, 'paged'=>$paged, 'tax_query'=>array($current_term, $deposit, $withdrawal, $type));
					query_posts($args);
