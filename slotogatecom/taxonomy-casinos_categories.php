			...
			<div class="align">
				<section>
					<header>
						...
						<h3><?php echo vsprintf(__('%s Reviews & Ratings in %s', 'websitelangid'), array($queried_object->name, $country_native_name));?></h3>
						<p><?php echo __('Please Rate Our Casinos Collection', 'websitelangid');?></p>
						<?php echo do_shortcode('[gdrts_stars_rating type="terms.casinos_categories" id="'.$term_id_original.'"]');?>
					</header>
					<?php
					if(is_paged()){
						if($_POST){
