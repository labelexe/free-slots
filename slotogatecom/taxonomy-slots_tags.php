				...
				<section>
					<div class="taxonomy-archive-intro taxonomy-archive-intro-align-top">
						<div class="taxonomy-archive-content">
							<?php if($term_id == 700){echo '<img width="100" height="42" src="'.get_bloginfo('template_url').'/images/icons/new-icon.svg" alt="New Icon" loading="lazy"/>';}?>
	...
	owl_id_provider_slots_archive_slider.owlCarousel({
		...
		<div class="content-zone archive-listings float">
			<div class="align">
				<section>
					<header>
						<h3><?php echo sprintf(__('%s Machine Games', 'websitelangid'), $queried_object->name);?></h3>
						<p><?php echo __('Please Rate this Collection', 'websitelangid');?></p>
						<?php echo do_shortcode('[gdrts_stars_rating type="terms.slots_tags" id="'.$term_id_original.'"]');?>
					</header>
					<?php
					if(is_paged()){
						if($_POST){
...
jQuery(function(){
	jQuery(".clear-btn").click(function(){
		...
		jQuery('.slots-filter').submit();
					...
					$args = array('post_type'=>'slots', 'meta_key'=>$slots_meta_key, 'orderby'=>$slots_orderby, 'order'=>$slots_order, 'paged'=>$paged, 'tax_query'=>array($current_term, $provider, $theme, $feature));
					query_posts($args);
					?>
					<?php if(have_posts()):?>
