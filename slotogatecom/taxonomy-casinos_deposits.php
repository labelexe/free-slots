				...
				<section>
					<div class="taxonomy-archive-intro">
						<div class="taxonomy-archive-content">
							...
							<?php }else{?>
								<?php echo vsprintf(__('<h2>%s</h2><p>%s is a payment method, used to deposit funds. To start a gameplay in an online casino, you will need to place a deposit into your bank account via %s. Funds are usually processed within several days. You just need to check if there are any extra charges imposed by %s and the selected online casino.</p>', 'websitelangid'), array($term_name, $term_name, $term_name, $term_name));?>
							<?php }?>
						</div>
			...
			<div class="align">
				<section>
					<header>
						...
						<h1><?php echo vsprintf(__('Casinos with %s Deposit Method in %s', 'websitelangid'), array($queried_object->name, $country_native_name));?></h1>
						<p><?php echo __('Please Rate Our Casinos Collection', 'websitelangid');?></p>
						<?php echo do_shortcode('[gdrts_stars_rating type="terms.casinos_deposits" id="'.$term_id_original.'"]');?>
					</header>
					<?php
					if(is_paged()){
						if($_POST){
...
jQuery(function(){
	jQuery(".clear-btn").click(function(){
		...
		jQuery('.casinos-filter').submit();
					...
					$args = array('post_type'=>'casinos', 'meta_key'=>$casinos_meta_key, 'orderby'=>$casinos_orderby, 'order'=>$casinos_order, 'paged'=>$paged, 'tax_query'=>array($current_term, $country, $currency, $withdrawal, $type));
					query_posts($args);
...
?>
		...
		<div class="content-zone linking-line float">
			<div class="align">
				<aside>
					<header>
						<h5><?php echo __('Other Casino Deposit Methods', 'websitelangid');?></h5>
					</header>
