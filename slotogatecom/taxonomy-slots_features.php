				...
				<section>
					<div class="taxonomy-archive-intro taxonomy-archive-intro-align-top">
						<div class="taxonomy-archive-content">
							...
							<?php }else{?>
								<?php echo vsprintf(__('<h2>%s Slot Games</h2><p>%s slot games involve the scatter that unlocks various bonus features. It doesn’t need to land on a payline to guarantee a win. In %s slot games, it just needs to land on the reels to activate multipliers or free spins. The rules for different %s slot games may vary, so it’s worth checking the paytable before you start playing.</p>', 'websitelangid'), array($term_name, $term_name, $term_name, $term_name));?>
							<?php }?>
						</div>
...
jQuery(function(){
	jQuery(".clear-btn").click(function(){
		...
		jQuery('.slots-filter').submit();
					...
					$args = array('post_type'=>'slots', 'meta_key'=>$slots_meta_key, 'orderby'=>$slots_orderby, 'order'=>$slots_order, 'paged'=>$paged, 'tax_query'=>array($current_term, $provider, $theme, $type));
					query_posts($args);
					?>
					<?php if(have_posts()):?>
...
?>
		...
		<div class="content-zone linking-line float">
			<div class="align">
				<aside>
					<header>
						<h5><?php echo __('Other Slots Features', 'websitelangid');?></h5>
					</header>
