				...
				<section>
					<div class="taxonomy-archive-intro">
						<div class="taxonomy-archive-content">
							...
							<?php }else{?>
								<?php echo vsprintf(__('<h2>%s</h2><p>%s is a payment method, which can be used for withdrawals. It will let you approach your winnings collected during the gameplay. Using %s, players are allowed to withdraw cash within certain limits. All transactions are processed within a certain period of time. Just don’t forget to check extra charges imposed by %s and the selected online casino.</p>', 'websitelangid'), array($term_name, $term_name, $term_name, $term_name));?>
							<?php }?>
						</div>
			...
			<div class="align">
				<section>
					<header>
						...
						<h1><?php echo vsprintf(__('Casinos with %s Withdrawal Method in %s', 'websitelangid'), array($queried_object->name, $country_native_name));?></h1>
						<p><?php echo __('Please Rate Our Casinos Collection', 'websitelangid');?></p>
						<?php echo do_shortcode('[gdrts_stars_rating type="terms.casinos_withdrawals" id="'.$term_id_original.'"]');?>
					</header>
					<?php
					if(is_paged()){
						if($_POST){
							...
							$FilterSessionSave_currency = $_SESSION['currency'];
	...
	jQuery('#ToggleFiltersContent').toggleClass('active');
						...
						<div class="filter-selects">
							<div class="filter-selects-wrapper filter-selects-4-cols">
								...
								<?php }?>
								<?php $casinos_currencies = get_terms(array('taxonomy'=>'casinos_currencies')); if($casinos_currencies){?>
								<div>
									<p>
										<label><?php echo __('Currency', 'websitelangid');?>:</label>
										<select name="currency">
											<option value="" <?php if($FilterSessionSave_currency == '' or $FilterSessionSave_currency == 0){echo 'selected="selected"';}?>><?php echo __('All Currencies', 'websitelangid');?></option>
											<?php foreach($casinos_currencies as $casinos_currency){echo '<option value="'.$casinos_currency->term_id.'"'; if($FilterSessionSave_currency == $casinos_currency->term_id){echo 'selected="selected"';} echo '>'.$casinos_currency->name.'</option>';}?>
										</select>
									</p>
								</div>
								<?php }?>
...
jQuery(function(){
	jQuery(".clear-btn").click(function(){
		...
		jQuery('select[name="currency"]', '.casinos-filter').val('').prop('selected', false);
		...
		jQuery('.casinos-filter').submit();
					...
					/*CURRENCY*/
					if($FilterSessionSave_currency != '' or $FilterSessionSave_currency != 0){
						$currency = array('taxonomy'=>'casinos_currencies', 'field'=>'id', 'terms'=>array($FilterSessionSave_currency));
					}
					...
					$args = array('post_type'=>'casinos', 'meta_key'=>$casinos_meta_key, 'orderby'=>$casinos_orderby, 'order'=>$casinos_order, 'paged'=>$paged, 'tax_query'=>array($current_term, $country, $deposit, $currency, $type));
					query_posts($args);
...
?>
		...
		<div class="content-zone linking-line float">
			<div class="align">
				<aside>
					<header>
						<h5><?php echo __('Other Casino Withdrawal Methods', 'websitelangid');?></h5>
					</header>
