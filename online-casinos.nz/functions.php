...
add_action( 'wp_enqueue_scripts', function() {
	...
	// wp_enqueue_script( 'slots', get_template_directory_uri() . '/assets/jquery/slots-filter.js', array(), 'null', true );
} );

add_action( 'wp_enqueue_scripts', 'true_include_myscript_dep_jquery', 25 );
 
function true_include_myscript_dep_jquery() {
	wp_enqueue_script( 'myscript', get_stylesheet_directory_uri() . '/assets/jquery/slots-filter.js', array( 'jquery' ), '3.0' );
}
add_action( 'wp_head', function () {
	$fonts_dir = get_template_directory_uri() . '/assets/';
	?>

<link rel="preload" href="<?php echo get_template_directory_uri(); ?>/assets/css/fonts.css" as="font" type="font/ttf" crossorigin>
...
function custom_meta($desc){
	...
	//Post Type: Casinos
	if(is_singular('casinos')){
		$get__yoast_wpseo_description = get_post_meta(get_the_ID(), '_yoast_wpseo_description', true);
		if(empty($get__yoast_wpseo_description)){
